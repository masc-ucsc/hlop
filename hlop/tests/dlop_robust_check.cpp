...
std::vector<BinOp> binops() {
  return {
      ...
      {"get_mask_op_range", [](const SP& a, const SP& b) { return a->get_mask_op_opt(0, std::max(1, b->get_signed_bits())); }},
      {"set_mask_op_range", [](const SP& a,                            const SP& b) { return a->set_mask_op_opt(3, 68, *b); }},
