...
void check_unbounded_pair(int64_t a, int64_t b) {
  ...
  for (int shamt : {0, 1, 7, 31, 63}) {
    if (a >= 0) {
      expect_unbounded_equal("lsh", a, shamt, *d_a->lsh_op(shamt), s_a.lsh_op(shamt), l_a.lsh_op(shamt));
    }
    expect_unbounded_equal("rsh", a, shamt, *d_a->rsh_op(shamt), s_a.rsh_op(shamt), l_a.rsh_op(shamt));
  }
